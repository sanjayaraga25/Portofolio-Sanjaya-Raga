...
@section('content')
    ...
    <div class="max-w-2xl glass rounded-2xl p-8">
        <h2 class="text-2xl font-display font-bold mb-6">Edit Service</h2>
        ...
        <form method="POST" action="{{ route('admin.services.update', $service) }}" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="icon" class="input-field @error('icon') border-danger @enderror">
                        <option value="code" {{ (old('icon', $service->icon)) === 'code' ? 'selected' : '' }}>Code</option>
                        <option value="camera" {{ (old('icon', $service->icon)) === 'camera' ? 'selected' : '' }}>Camera</option>
                        <option value="video" {{ (old('icon', $service->icon)) === 'video' ? 'selected' : '' }}>Video</option>
                        <option value="palette" {{ (old('icon', $service->icon)) === 'palette' ? 'selected' : '' }}>Palette</option>
                    </select>
            ...
            <div>
                ...
                <textarea name="description" required rows="4"
                    class="input-field resize-none @error('description') border-danger @enderror">{{ old('description', $service->description) }}</textarea>
                @error('description') <p class="text-danger text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="flex gap-3 pt-6">
                <button type="submit" class="btn-primary magnetic-btn">Update Service</button>
